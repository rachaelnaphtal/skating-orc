...
def generateReport(event: ValueChangeEventArguments):
    ...
    if report_params.type == ReportType.COMPETITION_REPORT:
        ...
        loop.run_until_complete(downloadResults.scrape(report_params.competition_url, report_params.report_name, event_regex=report_params.event_regex))
    else:
...
ui.label("Competition Results Number URL (ending in the number). For example: https://ijs.usfigureskating.org/leaderboard/results/2025/34240")
ui.input(placeholder='Ex: https://ijs.usfigureskating.org/leaderboard/results/2025/34240',
        validation={'Input wrong length': lambda value: len(value) != 63}).props('flat dense width=300').bind_value(report_params, 'competition_url')
ui.label("Name of report")
ui.input(placeholder='',
        validation={'Report Name': lambda value: len(value) < 1 }).props('flat dense width=300').bind_value(report_params, 'report_name')
ui.label("Regex for events. For all use '.*'.")
ui.input(placeholder='',
        validation={'Input too short': lambda value: len(value) <3 }).props('flat dense width=300').bind_value(report_params, 'event_regex')
